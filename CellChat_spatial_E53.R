...
spatial <- RenameIdents(spatial, '0' = 'Fibro1','1' = 'Perichondrium','2' = 'Muscles1')
spatial <- RenameIdents(spatial, '3' = 'Tenocytes', '4' ='Mes1', '5' = 'Fibro2')
spatial <- RenameIdents(spatial, '6' = 'Chondro', '7' = 'Mes2')
...
save(spatial, file = "visium_53_02.RData")


load("visium_53_02.RData")
library(Seurat)
spatial
...
cluster_colors <- c("Mes1" = "#a45c40", "Mes2" = "#b97d60", "Fibro1" = "#3d550c", "Muscles1"="pink",
                    "Fibro2" = "darkgreen", "Chondro" = "blue","Perichondrium" = "darkblue", "Tenocytes"="#ef7c8e" )

Seurat::SpatialDimPlot(spatial, label = T, label.size = 3, cols = cluster_colors, pt.size.factor = 6)
...
scale.factors = jsonlite::fromJSON(txt = file.path("/Users/gayanisenevirathne/Desktop/Desktop/Single_cell_Data_Analysis/Ilium_Multiomics_Analysis/Multiomics_E53_IL/outs_53_02/spatial", 'scalefactors_json.json'))
...
#Chondrocytes
netVisual_bubble(cellchat, sources.use = 1, targets.use = c(1:8), remove.isolate = FALSE)
netVisual_bubble(cellchat, sources.use = 7, targets.use = c(1:8), remove.isolate = FALSE)
netVisual_bubble(cellchat, sources.use = 2, targets.use = c(1:8), remove.isolate = FALSE)
netVisual_bubble(cellchat, sources.use = 3, targets.use = c(1:8), remove.isolate = FALSE)


#to save the graphs as a .pdf use this command
p <- netVisual_bubble(cellchat, sources.use = 7, targets.use = c(1:8), remove.isolate = FALSE)
p + theme(plot.margin = margin(1, 1, 1, 1, "cm"))  # Adjust margins
ggsave("bubble_plot.pdf", plot = p, width = 10, height = 25, dpi = 300)
...
par(mfrow=c(1,1))
par(mfrow=c(1,1))
netVisual_aggregate(cellchat, signaling = pathways.show, point.size = 4, layout = "spatial", edge.width.max = 2, vertex.size.max = 1, alpha.image = 0.2, vertex.label.cex = 3.5)
...
par(mfrow=c(1,1))
netVisual_aggregate(cellchat, signaling = pathways.show, layout = "spatial", point.size = 4, edge.width.max = 2, alpha.image = 0.5, vertex.weight = "incoming", vertex.size.max = 3, vertex.label.cex = 3.5)
...
spatialFeaturePlot(cellchat, features = c("WNT5A","FZD9"), point.size = 2.0, color.heatmap = "Reds", direction = 1)
spatialFeaturePlot(cellchat, features = c("WNT11","FZD9"), point.size = 2.0, color.heatmap = "Reds", direction = 1)

spatialFeaturePlot(cellchat, features = c("TGFA","ERBB2"), point.size = 2.0, color.heatmap = "Reds", direction = 1)

spatialFeaturePlot(cellchat, features = c("SEMA3C","PLXNA1"), point.size = 2.0, color.heatmap = "Reds", direction = 1)
spatialFeaturePlot(cellchat, features = c("SEMA3C","PLXNA1"), point.size = 2.0, color.heatmap = "Reds", direction = 1)

spatialFeaturePlot(cellchat, features = c("MDK","PTPRZ1"), point.size = 2.0, color.heatmap = "Reds", direction = 1)
spatialFeaturePlot(cellchat, features = c("MDK","SDC4"), point.size = 2.0, color.heatmap = "Reds", direction = 1)

spatialFeaturePlot(cellchat, features = c("GDF11","TGFBR1"), point.size = 2.0, color.heatmap = "Reds", direction = 1)
spatialFeaturePlot(cellchat, features = c("GDF11","ACVR2A"), point.size = 2.0, color.heatmap = "Reds", direction = 1)

spatialFeaturePlot(cellchat, features = c("FGF2","FGFR3"), point.size = 2.0, color.heatmap = "Reds", direction = 1)

spatialFeaturePlot(cellchat, features = c("FGF9","FGFR4"), point.size = 2.0, color.heatmap = "Reds", direction = 1)

spatialFeaturePlot(cellchat, features = c("VEGFA","VEGFR"), point.size = 2.0, color.heatmap = "Reds", direction = 1)


spatialFeaturePlot(cellchat, features = c("PTHLH","PTH1R"), point.size = 2.0, color.heatmap = "Reds", direction = 1)
spatialFeaturePlot(cellchat, features = c("IHH","PTCH2"), point.size = 1.5, color.heatmap = "Reds", direction = 1)
spatialFeaturePlot(cellchat, features = c("WNT5A","FZD9"), point.size = 1.4, color.heatmap = "Reds", direction = 1)
spatialFeaturePlot(cellchat, features = c("PTN","SDC4"), point.size = 2.0, color.heatmap = "Reds", direction = 1)
spatialFeaturePlot(cellchat, features = c("PTN","SDC2"), point.size = 2.0, color.heatmap = "Reds", direction = 1)

spatialFeaturePlot(cellchat, features = c("PTN","SDC2"), point.size = 2.0, color.heatmap = "Reds", direction = 1)
spatialFeaturePlot(cellchat, features = c("MDK","NCL"), point.size = 1.5, color.heatmap = "Reds", direction = 1)
